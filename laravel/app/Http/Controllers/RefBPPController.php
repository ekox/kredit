...
class RefBPPController extends Controller {
	...
	*/
	...
	public function index(Request $request)
	{
		try{
			...
			$aColumns = array('kdbpp','nmbpp');
			/* Indexed column (used for fast and accurate table cardinality) */
			$sIndexColumn = "kdbpp";
			/* DB table to use */
			$sTable = "SELECT kdbpp,nmbpp FROM t_bpp WHERE kdsatker='".session('kdsatker').
				"' AND thang='".session('tahun')."' ORDER BY kdbpp";
			/*
			 * Paging
			 */
			$sLimit = "";
	...
	public function dropdown()
	{
		try{
			...
			if(count($rows)>0){
				...
				foreach($rows as $row){
					$data .= '<option value="'.$row->kdbpp.'">'.$row->nmbpp.'</option>';
				}
	...
	/**
	 * upload data bpp ke direktory data/adk/referensi 
	 */
				...
				if(in_array($fileParts['extension'],$fileTypes)) {
					...
					if($fileSize>0){
						...
						if(file_exists($targetFolder.$file_name_baru)){
							session(['upload_ref_bpp' => $file_name_baru]);
							return '1';
						}
						else{
	...
	/**
	 * simpan data referensi bpp ke tabel 
	 */
	public function simpan(Request $request)
	{
		try{
			...
			if(session('upload_ref_bpp')!==null){
				
				$path = 'data/adk/referensi/'.session('upload_ref_bpp');
				$arr_file = explode(".", session('upload_ref_bpp'));
				...
				return 'Anda belum mengupload file ADK untuk BPP!';
			}
		}
		catch(\Exception $e){
			return $e;
