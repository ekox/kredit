	...
	*/
			...
			if(!isset($_GET['kdgiat'])) {
				...
				$sTable = "SELECT 
							...
							WHERE kdgiat IN 
							  ...
							  FROM
							    t_giat 
							  WHERE kddept = '023' AND kdgiat= '".$_GET['kdgiat']."') 
							ORDER BY 1,
							  2 ";				
			}
	...
	public function dropdown()
	{
		try{
			...
			if(count($rows)>0){
				...
				foreach($rows as $row){
					$data .= '<option value="'.$row->kdoutput.'">'.$row->kdgiat.'-'.$row->kdoutput.' - '.$row->nmoutput.'</option>';
				}
	...
	public function dipa_dropdown()
	{
		try{
			...
			$rows = DB::select("
				SELECT	a.*,
					IFNULL(b.nmoutput,'Tidak ada referensi') AS nmoutput
				FROM(
					SELECT	DISTINCT
						kdgiat,
						kdoutput
				...
				LEFT OUTER JOIN t_output b ON(a.kdgiat=b.kdgiat AND a.kdoutput=b.kdoutput)
