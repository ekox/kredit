...
class RefKabKotaController extends Controller {
	...
	*/
	...
	public function dropdown()
	{
		try{
			$rows = DB::select("
				select	distinct kdkabkota,nmkabkota
				from t_kabkota
				WHERE kdkabkota<>'00'
				order by kdkabkota asc
			");
			...
			if(count($rows)>0){
				...
				foreach($rows as $row){
					$data .= '<option value="'.$row->kdkabkota.'">'.$row->nmkabkota.'</option>';
				}
	...
	public function dropdown_by_id($kdlokasi)
	{
		try{
			$rows = DB::select("
				select	distinct kdkabkota,nmkabkota
				from t_kabkota
				WHERE kdkabkota<>'00' AND kdlokasi=?
				order by kdkabkota asc
			",[
				$kdlokasi
			]);
