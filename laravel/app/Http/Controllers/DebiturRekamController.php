															...
															if($rows[0]->nmdok==''){
																...
																if($usia>=17){*/
																	...
																	$insert = DB::insert("
																		...
																		kdkelamin,kdagama,kdpendidikan,kdpekerjaan,kdkawin,stsbpjs,kdbpjs,nohp,email,jmltinggal,
																	...
																	",[
																		...
																		''/*$data['kdkelamin']*/,''/*$data['kdagama']*/,$data['kdpendidikan'],$data['kdpekerjaan'],$data['kdkawin'],$data['stsbpjs'],$data['kdbpjs'],$data['nohp'],$data['email'],preg_replace("/[^0-9 \d]/i", "", $data['jmltinggal']),
																	...
																	if($insert){
																		...
																		if($request->input('is_alamat_ktp')=='0'){
																			...
																			if($valid){
																				...
																				$insert2 = DB::insert("
																					insert into d_debitur_alamat
																					(nik,kdalamat,kdprop,kdkabkota,kdkec,kdkel,kodepos,telp,alamat,stsdomisili,ketstsdomisili,created_at,updated_at)
																					values(?,'2',?,?,?,?,?,?,?,?,?,now(),now())
																				",[
																					$data['nik'],$data['kdprop1'],$data['kdkabkota1'],$data['kdkec1'],
																					$data['kdkel1'],$data['kodepos1'],$data['telp1'],$data['alamat1'],
																					$data['stsdomisili'],$data['ketstsdomisili']
																				]);
																			}
																			else{
																				$insert2 = false;
																		...
																		if($insert2){
																			...
																			if($request->input('kdkawin')=='1'){
																				...
																				",[
																					...
																					''/*$data['kdkelamin_p']*/,''/*$data['kdagama_p']*/,$data['kdpekerjaan_p'],$data['kdpendidikan_p'],$data['nohp_p'],''/*$data['email_p']*/,$data['kdprop_p'],$data['kdkabkota_p'],$data['kdkec_p'],$data['kdkel_p'],$data['kodepos_p'],$data['telp_p'],$data['alamat_p'],$data['is_alamat_pemohon']
																				]);
																				...
																				if($insert){
																					...
																					if($request->input('kdtipe')=='2'){
																						...
																						if($valid){
																							...
																							",[
																								...
																								preg_replace("/[^0-9 \d]/i", "", $data['lama_k_p']),
																								preg_replace("/[^0-9 \d]/i", "", $data['penghasilan_p']),
																								preg_replace("/[^0-9 \d]/i", "", $data['penghasilan1_p'])
																							]);
																					...
																					if($lanjut){
																						...
																						if($lanjut){
																							...
																							if(count($request->input('kdkreditur'))>0){
																								
																								$arr_utang = $request->input('kdhutang');
																								$arr_kreditur = $request->input('kdkreditur');
																								$arr_jmlunit = $request->input('jmlunit');
																								$arr_total = $request->input('total');
																								...
																								for($i=0;$i<$jml_key;$i++){
																									...
																									if($arr_kreditur[$arr_key[$i]]!=='' && $arr_kreditur[$arr_key[$i]]!==null){
																										
																										$tot_utang++;
																										
																										if($arr_jmlunit[$arr_key[$i]]>0 && $arr_total[$arr_key[$i]]>0 && $arr_angsuran[$arr_key[$i]]>0){
																											$query_utang[] = "('".$data['nik']."',
																																".$arr_key[$i].",
																																".$arr_kreditur[$arr_key[$i]].",
																																".preg_replace("/[^0-9 \d]/i", "", $arr_jmlunit[$arr_key[$i]]).",
																																".preg_replace("/[^0-9 \d]/i", "", $arr_total[$arr_key[$i]]).",
																																".preg_replace("/[^0-9 \d]/i", "", $arr_angsuran[$arr_key[$i]]).")";
																										}
