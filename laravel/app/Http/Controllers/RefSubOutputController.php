...
class RefSubOutputController extends Controller {
	...
	*/
	...
	public function dipa_dropdown1($kdppk)
	{
		try{
			...
			$rows = DB::select("
				SELECT	a.*,
						IFNULL(b.ursoutput,'Tidak ada referensi') AS ursoutput
				FROM(
					SELECT	DISTINCT
						kdgiat,
						kdoutput,
				...
				LEFT OUTER JOIN t_soutput b ON(a.kdgiat=b.kdgiat AND a.kdoutput=b.kdoutput AND a.kdsoutput=b.kdsoutput)
				ORDER BY a.kdgiat,a.kdoutput,a.kdsoutput
			",[
				session('kdsatker'),
				session('tahun')
			]);
			...
			if(count($rows)>0){
				...
				$data = '<option value="" style="display:none;">Pilih Data</option>';
				foreach($rows as $row){
					$data .= '<option value="'.$row->kdsoutput.'">'.$row->kdgiat.'-'.$row->kdoutput.' - '.$row->kdsoutput.' - '.$row->ursoutput.'</option>';
				}
