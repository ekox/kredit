...
class RefProgramController extends Controller {
	...
	*/
	...
	{
		try{
			$aColumns = array('kddept','kdunit','kdprogram','nmprogram');
	...
	public function dipa_dropdown()
	{
		try{
			$rows = DB::select("
				  SELECT	a.*,
							IFNULL(b.nmprogram,'Tidak ada referensi') AS nmprogram
					FROM(
						SELECT	DISTINCT 
							kddept,
							kdunit,
							kdprogram
						FROM d_pagu
						WHERE lvl='7' AND kdsatker=? AND thang=? AND kdppk=?
					) a
					LEFT OUTER JOIN t_program b ON(a.kddept=b.kddept AND a.kdunit=b.kdunit AND a.kdprogram=b.kdprogram)
			...
			if(count($rows)>0){
				...
				foreach($rows as $row){
					$data .= '<option value="'.$row->kdprogram.'">'.$row->kdprogram.' - '.$row->nmprogram.'</option>';
				}
