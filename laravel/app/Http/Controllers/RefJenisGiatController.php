...
class RefJenisGiatController extends Controller {
	...
	*/
	...
	public function index(Request $request)
	{
		try{
			...
			$aColumns = array('jenisgiat','nmjenisgiat');
			/* Indexed column (used for fast and accurate table cardinality) */
			$sIndexColumn = "jenisgiat";
			/* DB table to use */
			// $sTable = "SELECT	a.id,
			// 					b.nmalur,
			// 					c.nmjenisgiat,
	...
	public function dropdown()
	{
		try{
			...
			if(count($rows)>0){
				...
				foreach($rows as $row){
					$data .= '<option value="'.$row->jenisgiat.'">'.$row->nmjenisgiat.'</option>';
				}
